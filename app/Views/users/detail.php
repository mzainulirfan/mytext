    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form class="row g-3" method="post" action="<?= base_url(); ?>user/update">
          ...
          <div class="col-12">
            <label for="address" class="form-label">Address</label>
            <textarea class="form-control <?= (session()->has('validation') && ($validation = session('validation'))->hasError('address')) ? 'is-invalid' : '' ?>" id="content" name="address" rows="5" <?= (session()->has('validation') && ($validation = session('validation'))->hasError('address')) ? 'autofocus' : '' ?>><?= (old('address')) ? old('address') : esc($userData['user_address']); ?></textarea>
            <div class="invalid-feedback">
              <?php
